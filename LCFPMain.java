...
public class LCFPMain {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Create Datacenter (same as SCFP)
            Datacenter datacenter0 = createDatacenter("Datacenter_0");

            // Create Broker
            LCFPBroker broker = new LCFPBroker("LCFP_Broker");
            int brokerId = broker.getId();

            // Create VMs (same MIPS as SCFP)
            List<Vm> vmList = new ArrayList<>();
            int[] mipsValues = {1000, 2500, 4000}; // same as SCFP
            ...
            // Create Cloudlets (same lengths as SCFP)
            ...
            for (int i = 0; i < numCloudlets; i++) {
                int length = 5000 + rand.nextInt(15000); // 5000–20000
    ...
    // Datacenter configuration (same as SCFP)
    private static Datacenter createDatacenter(String name) {
        List<Host> hostList = new ArrayList<>();
        List<Pe> peList = new ArrayList<>();

        // Each PE has 4000 MIPS to accommodate VMs
        peList.add(new Pe(0, new PeProvisionerSimple(4000)));
        peList.add(new Pe(1, new PeProvisionerSimple(4000))); // total 8000 MIPS
